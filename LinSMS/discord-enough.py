...
async def on_ready():
    ...
    activity = discord.Activity(type=discord.ActivityType.playing, name="Lino Yazılım Hizmetleri")
...
async def on_message(message):
    ...
    if len(message.content.split(" ")) == 2 and message.content.split(" ")[0] == "*sms":
        if len(message.content.split(" ")[1]) == 10:
            ...
            embed=discord.Embed(title="LinSMS", description=(f"{adet} adet SMS Gönderilmeye başlandı --> {telno}\n{message.author.mention}"), color=0x001eff)
        ...
        await message.channel.send(f"Hocam komut basit.\n```*sms 5051234567```\n*sms (telefon numarası)\n{message.author.mention}")
    else:
        pass
